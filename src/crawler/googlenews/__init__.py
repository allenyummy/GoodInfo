# encoding=utf-8
# Author: Iceloof (https://github.com/Iceloof/GoogleNews)
# Description: Get news from google
...
def lexical_date_parser(date_to_check):
    ...
    count = 0
    while datetime_tmp is None and count <= (len(date_to_check) - 3):
...
class GoogleNews:
    ...
    def search(self, key):
        """
        Searches for a term in google.com in the news section,
        and retrieves the first page into __results.
        Parameters:
        key = the search term
        """
    ...
    def build_response(self):
        self.req = urllib.request.Request(
            self.url.replace("search?", "search?hl=en&gl=en&"), headers=self.headers
        )
    ...
    def page_at(self, page=1):  # noqa: C901
        """
        ...
        try:
            if self.__start != "" and self.__end != "":
                self.url = (
                    ...
                    "&bih=976"
                    "&source=lnt"
                    "&&tbs=lr:lang_1{},cdr:1,cd_min:{},cd_max:{},sbd:1"
                    "&tbm=nws"
                    "&start={}".format(
                        self.__key,
                        self.__lang,
        ...
        try:
            ...
            for item in result:
                ...
                try:
                    tmp_date = (
                        item.find("div", {"role": "heading"})
                        .next_sibling.findNext("div")
                        .findNext("div")
                        .text
                    )
            ...
            logger.debug(e_parser)
            pass

        return results

    def get_page(self, page=1):  # noqa: C901
        """
        Retrieves a specific page from google.com in the news sections into __results.
        Parameter:
        page = number of the page to be retrieved
    ...
    def get_news(self, key="", deamplify=False):  # noqa: C901
        if key != "":
            key = "+".join(key.split(" "))
            if self.__encode != "":
                key = urllib.request.quote(key.encode(self.__encode))
            self.url = "https://news.google.com/search?q={}+when:{}&hl={}".format(
            ...
            for article in articles:
                try:
                    ...
                    try:
                        ...
                        datetime_obj = parse(datetime_chars).replace(  # noqa: F841
                            tzinfo=None
                        )
                    except Exception:
                        datetime_obj = None  # noqa: F841
                    # link
                    if deamplify:
                        try:
                            link = "news.google.com/" + article.find("h3").find(
                                "a"
                            ).get("href")
                        except Exception as deamp_e:
                            logger.debug(deamp_e)
                            link = (
                        ...
                        link = "news.google.com/" + article.find("h3").find("a").get(
                            "href"
                        )
